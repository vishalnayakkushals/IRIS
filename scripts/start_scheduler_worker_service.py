...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Start IRIS core scheduler worker without Docker")
    parser.add_argument("--poll-seconds", type=int, default=int(os.getenv("IRIS_SCHEDULER_POLL_SECONDS", "10") or 10))
...
def main() -> None:
    ...
    command = [
        sys.executable,
        str(repo_root / "scripts" / "scheduler_worker.py"),
        "--db",
        str(runtime["db_path"]),
        "--root",
        str(runtime["stores_root"]),
        "--out",
        str(runtime["exports_dir"]),
        "--poll-seconds",
    ...
    print(
        f"[iris-scheduler] env={env_file or 'none'} db={runtime['db_path']} "
        f"root={runtime['stores_root']} out={runtime['exports_dir']} poll={int(args.poll_seconds)}"
    )
