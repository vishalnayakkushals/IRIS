...
try {
    ...
    Write-Host "[IRIS] Running normal launcher with secure key-file loading..."
    if ($null -eq $RunArgs -or $RunArgs.Count -eq 0) {
        & ".\run_iris.bat"
    } else {
        & ".\run_iris.bat" @RunArgs
    }
    if ($LASTEXITCODE -ne 0) {
        throw "run_iris.bat failed with exit code $LASTEXITCODE"
    }
    Write-Host "[IRIS] Normal run completed."
}
catch {
    Write-Error $_
    exit 1
